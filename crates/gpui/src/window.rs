...
impl Window {
    ...
    pub fn defer(&mut self, cx: &mut AppContext, f: impl FnOnce(&mut WindowContext) + 'static) {
    ...
    fn bounds_changed(&mut self, cx: &mut AppContext) {
        ...
        self.bounds_observers
            .clone()
            .retain(&(), |callback| callback(&mut WindowContext::new(cx, self)));
    }
    ...
    pub fn is_action_available(&self, action: &dyn Action, cx: &AppContext) -> bool {
        let target = self
            .focused(cx)
            ...
            || previous_window_active != current_window_active
        {
            if !previous_focus_path.is_empty() && current_focus_path.is_empty() {
                self.focus_lost_listeners
                    .clone()
                    .retain(&(), |listener| listener(&mut WindowContext::new(cx, self)));
            }
            ...
            self.focus_listeners.clone().retain(&(), |listener| {
                listener(&event, &mut WindowContext::new(cx, self))
            });
        }
    ...
    fn draw_roots(&mut self, cx: &mut AppContext) {
        let mut cx = WindowContext::new(cx, self);
        cx.window.draw_phase = DrawPhase::Prepaint;
        cx.window.tooltip_bounds.take();

        // Layout all root elements.
        let mut root_element = cx.window.root_view.as_ref().unwrap().clone().into_any();
        root_element.prepaint_as_root(Point::default(), cx.window.viewport_size.into(), &mut cx);

        let mut sorted_deferred_draws =
            (0..cx.window.next_frame.deferred_draws.len()).collect::<SmallVec<[_; 8]>>();
        sorted_deferred_draws.sort_by_key(|ix| cx.window.next_frame.deferred_draws[*ix].priority);
        cx.prepaint_deferred_draws(&sorted_deferred_draws);

        let mut prompt_element = None;
        let mut active_drag_element = None;
        let mut tooltip_element = None;
        if let Some(prompt) = cx.window.prompt.take() {
            let mut element = prompt.view.any_view().into_any();
            element.prepaint_as_root(Point::default(), cx.window.viewport_size.into(), &mut cx);
            prompt_element = Some(element);
            cx.window.prompt = Some(prompt);
        } else if let Some(active_drag) = cx.app.active_drag.take() {
            let mut element = active_drag.view.clone().into_any();
            let offset = cx.mouse_position() - active_drag.cursor_offset;
            element.prepaint_as_root(offset, AvailableSpace::min_size(), &mut cx);
            active_drag_element = Some(element);
            cx.app.active_drag = Some(active_drag);
        } else {
            tooltip_element = cx.prepaint_tooltip();
        }

        cx.window.mouse_hit_test = cx.window.next_frame.hit_test(cx.mouse_position());

        // Now actually paint the elements.
        cx.window.draw_phase = DrawPhase::Paint;
        root_element.paint(&mut cx);

        cx.paint_deferred_draws(&sorted_deferred_draws);

        if let Some(mut prompt_element) = prompt_element {
            prompt_element.paint(&mut cx);
        } else if let Some(mut drag_element) = active_drag_element {
            drag_element.paint(&mut cx);
        } else if let Some(mut tooltip_element) = tooltip_element {
            tooltip_element.paint(&mut cx);
        }
    }
    ...
    fn prepaint_tooltip(&mut self, cx: &mut AppContext) -> Option<AnyElement> {
        ...
        let tooltip_size = element.layout_as_root(
        ...
        self.with_absolute_element_offset(tooltip_bounds.origin, |window| {
            element.prepaint(&mut WindowContext::new(cx, window))
        });
    ...
    fn prepaint_deferred_draws(&mut self, deferred_draw_indices: &[usize], cx: &mut AppContext) {
        let mut cx = WindowContext::new(cx, self);
        assert_eq!(cx.window.element_id_stack.len(), 0);

        let mut deferred_draws = mem::take(&mut cx.window.next_frame.deferred_draws);
        for deferred_draw_ix in deferred_draw_indices {
            ...
            if let Some(element) = deferred_draw.element.as_mut() {
                ...
                cx.reuse_prepaint(deferred_draw.prepaint_range.clone());
            }
            let prepaint_end = cx.prepaint_index();
            deferred_draw.prepaint_range = prepaint_start..prepaint_end;
        }
        assert_eq!(
            cx.window.next_frame.deferred_draws.len(),
            0,
            "cannot call defer_draw during deferred drawing"
        );
        cx.window.next_frame.deferred_draws = deferred_draws;
        cx.window.element_id_stack.clear();
        cx.window.text_style_stack.clear();
    }
        ...
        task.clone().now_or_never().or_else(|| {
            if is_first {
                ...
                cx.spawn({
                    ...
                    |mut cx| async move {
                        task.await;
                        window_handle.update(&mut cx, |_, cx| {
    ...
    ///
    /// This method should only be called as part of the prepaint phase of element drawing.
    pub fn set_focus_handle(&mut self, focus_handle: &FocusHandle, cx: &mut AppContext) {
        debug_assert_eq!(
            self.draw_phase,
            DrawPhase::Prepaint,
            "this method can only be called during prepaint"
        );
        if focus_handle.is_focused(&WindowContext::new(cx, self)) {
    ...
    pub fn handle_input(
        ...
        if focus_handle.is_focused(&WindowContext::new(cx, self)) {
            let mut cx = AsyncWindowContext::new(cx.to_async(), self.handle);
            self.next_frame
                .input_handlers
    ...
    pub fn on_modifiers_changed(
        ...
        self.next_frame.dispatch_tree.on_modifiers_changed(Rc::new(
            move |event: &ModifiersChangedEvent, cx: &mut WindowContext<'_>| listener(event, cx),
        ));
    }
    ...
    fn reset_cursor_style(&mut self, cx: &mut AppContext) {
        // Set the cursor only if we're the active window.
        if self.is_window_hovered() {
            let mut cursor_styles = mem::take(&mut self.rendered_frame.cursor_styles);
            let style = cursor_styles
                .iter()
                .rev()
                .find(|request| request.hitbox_id.is_hovered(&WindowContext::new(cx, self)))
                .map(|request| request.style)
                .unwrap_or(CursorStyle::Arrow);
            self.rendered_frame.cursor_styles = cursor_styles;
    ...
    pub fn dispatch_keystroke(&mut self, keystroke: Keystroke, cx: &mut AppContext) -> bool {
        ...
        if let Some(input) = keystroke.key_char {
            if let Some(mut input_handler) = self.platform_window.take_input_handler() {
                let mut window_cx = WindowContext::new(cx, self);
                input_handler.dispatch_input(&input, &mut window_cx);
                ...
                FileDropEvent::Entered { position, paths } => {
                    ...
                    if cx.active_drag.is_none() {
                        cx.active_drag = Some(AnyDrag {
                            ...
                            view: cx.new_model(|_| paths).into(),
    ...
    fn dispatch_mouse_event(&mut self, event: &dyn Any, cx: &mut AppContext) {
        ...
        if cx.active_drag.is_some() {
            if event.is::<MouseMoveEvent>() {
                ...
                WindowContext::new(cx, self).refresh();
    ...
    fn replay_pending_input(&mut self, replays: SmallVec<[Replay; 1]>, cx: &mut AppContext) {
        ...
        'replay: for replay in replays {
            ...
            if let Some(input) = replay.keystroke.key_char.as_ref().cloned() {
                if let Some(mut input_handler) = self.platform_window.take_input_handler() {
                    input_handler.dispatch_input(&input, &mut WindowContext::new(cx, self));
                    self.platform_window.set_input_handler(input_handler)
                }
            }
        }
    ...
    pub fn observe_global<G: Global>(
        ...
        let (subscription, activate) = cx.global_observers.insert(
            TypeId::of::<G>(),
            Box::new(move |app_cx| window_handle.update(app_cx, |_, cx| f(cx)).is_ok()),
        );
        cx.defer(move |_| activate());
        subscription
    }
...
impl Context for WindowContext<'_> {
    type Result<T> = T;
    type EntityContext<'f, T: 'static> = ViewContext<'f, T>;

    fn new_model<T>(
        &mut self,
        build_model: impl for<'f> FnOnce(&mut ViewContext<'f, T>) -> T,
    ) -> Model<T>
    where
        T: 'static,
    {
        self.app.update(|cx| {
            ...
            let mut model_cx = &mut ModelContext::new(cx, &slot);
            let mut view_cx = &mut ViewContext::new(model_cx, self.window);
            let entity = build_model(view_cx);
    ...
    fn insert_model<T: 'static>(
        ...
        self.app.insert_model(reservation, |cx| {
            let mut cx = ViewContext::new(cx, window);
            build_model(&mut cx)
        })
        */
    }
    ...
    fn update_model<T: 'static, R>(
        &mut self,
        model: &Model<T>,
        update: impl for<'f> FnOnce(&mut T, &mut ViewContext<'f, T>) -> R,
    ) -> R {
        let mut entity = self.entities.lease(model);
        let result = update(
            &mut *entity,
            &mut ViewContext::new(&mut ModelContext::new(self.app, model), self.window),
        );
...
impl VisualContext for WindowContext<'_> {
    fn new_view<V>(
        &mut self,
        build_view_state: impl for<'f> FnOnce(&mut ViewContext<'f, V>) -> V,
    ) -> Self::Result<Model<V>>
    where
        V: 'static + Render,
    {
        ...
        let view = self
            .app
            .new_model(|cx| build_view_state(&mut ViewContext::new(cx, self.window)));
    ...
    fn update_view<T: 'static, R>(
        ...
        let mut model_cx = ModelContext::new(&mut *self.app, view);
        let mut cx = ViewContext::new(&mut model_cx, &mut *self.window);
    ...
    fn replace_root_view<V>(
        &mut self,
        build_view: impl for<'f> FnOnce(&mut ViewContext<'f, V>) -> V,
    ) -> Self::Result<Model<V>>
    where
        V: 'static + Render,
    {
    ...
    fn focus_view<V: crate::FocusableView>(&mut self, view: &Model<V>) -> Self::Result<()> {
        self.update_view(view, |view, cx| {
            view.focus_handle(cx).clone().focus(cx.window_context());
        })
    }
...
impl<'a, V: 'static> ViewContext<'a, V> {
    pub(crate) fn new(cx: &'a mut ModelContext<'a, V>, window: &'a mut Window) -> Self {
        Self { cx, window }
    }

    /// Get the entity_id of this view.
    pub fn entity_id(&self) -> EntityId {
        self.cx.entity.entity_id()
    }
        ...
        V: 'static,
    {
        ...
        self.window.on_next_frame(move |cx| view.update(cx, f));
    }
    ...
    pub fn defer(&mut self, f: impl FnOnce(&mut V, &mut ViewContext<V>) + 'static) {
        ...
        self.window.defer(&mut self.cx, move |cx| {
            view.update(cx, f).ok();
        });
    }
    ...
    pub fn observe<V2, E>(
        &mut self,
        entity: &E,
        mut on_notify: impl FnMut(&mut V, E, &mut ModelContext<'_, V>) + 'static,
    ) -> Subscription
    where
        V2: 'static,
        V: 'static,
    ...
    pub fn subscribe<V2, E, Evt>(
        &mut self,
        entity: &E,
        mut on_event: impl FnMut(&mut V, E, &Evt, &mut ModelContext<'_, V>) + 'static,
    ) -> Subscription
    where
        V2: EventEmitter<Evt>,
        E: Entity<V2>,
    ...
    pub fn observe_release<V2, E>(
        &self,
        entity: &E,
        mut on_release: impl FnMut(&mut V, &mut V2, &mut ModelContext<'_, V>) + 'static,
    ) -> Subscription
    where
        V: 'static,
        V2: 'static,
    ...
    pub fn notify(&mut self) {
        todo!()
        // self.cx.notify(Some(self.entity.entity_id()));
    }
        ...
        Fut: Future<Output = R> + 'static,
    {
        ...
        self.app
            .spawn(|cx| f(view, AsyncWindowContext::new(cx, window)))
    }

    /// Register a callback to be invoked when the given global state changes.
    pub fn observe_global<G: Global>(
        &mut self,
        mut f: impl for<'f> FnMut(&mut V, &mut ViewContext<'f, V>) + 'static,
    ) -> Subscription {
        let window_handle = self.window.handle;
        let view = self.view().downgrade();
        let (subscription, activate) = self.global_observers.insert(
    ...
    pub fn on_action(
        &mut self,
        action_type: TypeId,
        listener: impl for<'f> Fn(&mut V, &dyn Any, DispatchPhase, &mut ViewContext<'f, V>) + 'static,
    ) {
        todo!()
        /*
        let handle = self.view().clone();
        self.app.on_action(action_type, move |action, phase, cx| {
...
impl<V> Context for ViewContext<'_, V> {
    ...
    fn update_model<T: 'static, R>(
        ...
        todo!()
        // self.app.update_model(model, update)
    }
...
impl<V: 'static> VisualContext for ViewContext<'_, V> {
    fn new_view<W: Render + 'static>(
        ...
        self.window_context().new_view(build_view_state)
    }
    ...
    fn update_view<V2: 'static, R>(
        ...
        self.window_context().update_view(view, update)
    }
...
impl<V: 'static + Render> WindowHandle<V> {
    ...
    pub fn update<C, R>(
        &self,
        cx: &mut C,
        update: impl for<'f> FnOnce(&mut V, &mut ModelContext<'f, V>) -> R,
    ) -> Result<R>
    where
        C: Context,
    {
