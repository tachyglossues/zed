use gpui::{rems, Length, Rems, ViewContext};
...
pub fn vw<T: 'static>(percent: f32, cx: &mut ViewContext<T>) -> Length {
    Length::from(cx.window_context().viewport_size().width * percent)
}

/// Returns a [`Length`] corresponding to the specified percentage of the viewport's height.
///
/// `percent` should be a value between `0.0` and `1.0`.
pub fn vh<T: 'static>(percent: f32, cx: &mut ViewContext<T>) -> Length {
    Length::from(cx.window_context().viewport_size().height * percent)
}
